...
<div class="container">
 <div class="signin">
   <form method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>" name="page">
   		<h2>Sign In</h2>
	    العضو<input type="text" name="username" value="" placeholder="Enter Username" class="x">
        الرقم السرى<input type="password" name="userpassword" value="" placeholder="Enter Userpassword" class="x">
			       ...
			       function checkFunction($res ,$userpassword)
			    {
				      if ($res -> num_rows >0)
				      {
					       ...
					       //check if the user password is right 
					       	if($x==$userpassword ){
			                    // the user page
								header('REFRESH:1;URL= medical_host.php');
			                  }
